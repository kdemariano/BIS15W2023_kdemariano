...
```r
# No these values do not make sense. In amniota, NA values are actually imputted as -999
```
...
```r
#Yes for mammals it makes sense because they do not lay eggs. However for birds and mammals, they both lay eggs so there must be some underlying factor as to why there are NA values in the data.
```

**9. The `amphibio` data have variables that classify species as fossorial (burrowing), terrestrial, aquatic, or arboreal.Calculate the number of NA's in each of these variables. Do you think that the authors intend us to think that there are NA's in these columns or could they represent something else? Explain.**

```r
#The habitats are treated as their own variable rather than as an observation. NA are treated as if they do not live there rather than if the data was not collected
amphibio %>% 
  filter(is.na(Aqu)) %>% 
  count(Aqu)
```
