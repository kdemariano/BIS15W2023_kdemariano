...
penguins %>% 
  ...
  group_by(species) %>% 
  summarize(mean_bill_length= mean(bill_length_mm, na.rm=T), mean_bill_depth=mean(bill_depth_mm, na.rm=T), mean_fliper_length=mean(flipper_length_mm, na.rm=T), mean_body_mass=mean(body_mass_g, na.r=T))
```
...
penguins %>% 
  group_by(species) %>% 
  summarize(mean_bill_length= mean(bill_length_mm, na.r=T))
```
