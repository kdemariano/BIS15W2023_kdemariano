...
```r
deserts %>% 
  ggplot(aes(x=taxa,))+ geom_bar(na.rm=T)+ scale_y_log10()
```
...
deserts %>% 
  group_by(species) %>% 
  ggplot(aes(x=species, y=weight))+geom_boxplot(na.rm=T)+ geom_point()+ coord_flip()
```
...
deserts %>% 
  ...
  ggplot(aes(x=as.factor(year), y=n_samples)) + geom_col()+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) 
```
...
```r
deserts %>% 
  ggplot(aes(x=weight, y=hindfoot_length))+ geom_point(na.rm=T)+ scale_x_log10()+ geom_smooth(method=lm, se=T)
```
...
deserts %>% 
  ...
  filter(sex!="NA") %>% 
  filter(year=="1997") %>%  
  ggplot(aes(x=sex,y=weight, fill= species_id))+ geom_boxplot(na.rm=T) +
  labs(title= "Sex vs Weight in 1997", x="Sex", y="Weight")
```
