...
homerange %>% 
  group_by(class) %>% 
  ggplot(aes(x=class, y=log10.mass, fill=taxon))+ geom_boxplot(na.rm=T)
```
...
homerange %>% 
  ...
  geom_histogram(alpha=0.4,color = "dimgrey", fill = "coral3", bins=25)+
...
homerange %>% 
  ...
  geom_histogram(alpha=0.4,color = "dimgrey", fill = "coral3", bins=25)+labs((title= "Distribution or Log10.Hra")) 
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+ geom_histogram(aes(y=after_stat(density)), fill= "deepskyblue4", alpha=0.4, color= "black")+ geom_density(color="orange")+labs(title="Distriution of Homerange")
```
