...
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"),
                selectInput("x", "Select Taxon",
                            choices = c("birds","lake fishes", "lizards:", "mammals", "marine fishes", "river fishes", "snakes", "turtoises","turtles", selected="birds"),
                            plotOutput("plot", width="600px", height="500px")
                            )
...
ui <- dashboardPage(
  dashboardHeader(title= "Wolves Sex for Each Population"),
  dashboardSidebar(disable=T),
  dashboardBody(
    fluidRow(
...
server <- function(input, output, session) {
  output$plot<- renderPlot ({
   wolves %>% 
      ...
      filter(sex!="NA") %>% 
      count(sex) %>% 
      ggplot(aes(x=sex, y=input$y ,fill=sex))+ geom_col()
  })
  session$onSessionEnded(stopApp)
}
