...
```{r}
ggplot(homerange, mapping=aes(x=log10.mass, y=log10.hra))+geom_jitter() + geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(homerange, mapping=aes(x=log10.hra, y=log10.preymass))+geom_smooth(method=lm, se=T)
#It removes NA values
```
...
```{r}
homerange %>% 
  ggplot(aes(x=trophic.guild)) + #x is categorical
  geom_bar()
```
...
mammal_homerange %>% 
  top_n(10, log10.mass) %>% 
  ggplot(aes(x=common.name, y=log10.mass))+ geom_col() #+coord_flip to flip 
```
